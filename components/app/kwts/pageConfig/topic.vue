<template>
  <div class="cool">
    ...
    <Form :model="data" :label-width="70" style="margin-bottom: 80px;">
      <FormItem label="模块标题" required prop="title">
        <Input v-model="data.title" placeholder="模块标题"/>
      </FormItem>
      <FormItem label="模块背景" required prop="cover">
        <Upload
          action="/bridge/basic/upload?https=1"
          :show-upload-list="false"
          :on-success="data => $set(this.data, 'cover', data.url)">
          <Button icon="ios-cloud-upload-outline">Upload files</Button>
        </Upload>
      </FormItem>
      <FormItem label="背景地址" required prop="cover">
        <Input v-model="data.cover"/>
      </FormItem>
    </Form>
